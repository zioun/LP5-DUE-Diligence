...
export function transformVue(code, id, options) {
    try {
            ...
            nodeTransforms: [
                (node) => {
                    if (node.type === NodeTypes.ELEMENT && node.loc) {
                        addVueAttributes(node, id, s, options);
                    }
                },
            ],
        });
...
function addVueAttributes(node, filePath, s, options) {
...
function formatVueComponentContent(props) {
    try {
        // 过滤掉不需要的属性
        const filteredProps = Object.fromEntries(Object.entries(props).filter(([key]) => !key.startsWith('data-') &&
            !key.startsWith('v-') &&
            key !== 'key' &&
            key !== 'ref'));
